...
export default function Footer() {
  return (
    <footer id="contact" className="relative py-28 grain overflow-hidden"
      style={{ background: 'var(--bg)', borderTop: '1px solid var(--border)' }}>

      {/* Glow */}
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-[600px] h-[240px] pointer-events-none"
        style={{ background: 'radial-gradient(ellipse, rgba(139,124,246,0.07) 0%, transparent 70%)' }} />
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 mb-20">
          ...
          >
            ...
            <h2 className="font-extrabold tracking-[-0.03em] leading-[0.92] mb-6"
              style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', color: 'var(--text)' }}>
              Have a project
              <br />
              <span style={{ color: 'var(--faint)' }}>in mind?</span>
            </h2>
            <p className="text-sm leading-relaxed max-w-xs" style={{ color: '#6B6B80' }}>
          ...
          >
            {/* Contact cards */}
            <a href="mailto:mohammedhuzaifa857@gmail.com"
              className="group flex items-center gap-4 p-5 rounded-2xl transition-all duration-300"
              style={{ background: 'var(--card)', border: '1px solid var(--border)' }}
              onMouseEnter={(e) => {
                const el = e.currentTarget as HTMLAnchorElement
                el.style.borderColor = 'rgba(139,124,246,0.25)'
                el.style.background = 'rgba(139,124,246,0.04)'
              }}
              onMouseLeave={(e) => {
                const el = e.currentTarget as HTMLAnchorElement
            ...
            <div className="grid grid-cols-3 gap-3">
              {socials.map((s) => (
                ...
                >
                  <s.icon size={15} style={{ color: '#6B6B80' }} />
                  <span className="text-[11px] font-medium" style={{ color: '#6B6B80', fontFamily: 'var(--font-mono)' }}>
                    {s.label}
                  </span>
                </a>
              ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8"
          ...
          <p className="text-[11px]" style={{ color: 'var(--faint)', fontFamily: 'var(--font-mono)' }}>
            © {new Date().getFullYear()} Mohammed Huzaifa. Crafted with care.
          </p>
        </div>
      </div>
    </footer>
